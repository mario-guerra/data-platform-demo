...
def generate_synthetic_data(n_samples=1000):
    ...
    for _ in range(n_samples):
        ...
        price = (
            base_price * regional_multiplier[region] * (0.8 + 0.4 * np.random.random())
        )
...
def prepare_features(df):
    ...
    if "product_encoded" in features.columns and "region_encoded" in features.columns:
        features["product_region_interaction"] = (
            features["product_encoded"] * features["region_encoded"]
        )
...
def main():
    ...
    with mlflow.start_run():
        ...
        artifact_path = (
            f"/tmp/model_artifacts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.joblib"
        )
        ...
        if hasattr(model, "feature_importances_"):
            ...
            importance_plot_path = f"/tmp/feature_importance_{timestamp}.png"
        ...
        sample_predictions = pd.DataFrame(
            {"actual": y_test.head(10), "predicted": y_pred_test[:10]}
        )
