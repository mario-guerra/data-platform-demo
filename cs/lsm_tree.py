...
class MemTable:
    """
    In-memory sorted table using a balanced tree structure
    (simulated with OrderedDict)
    """
...
class SSTable:
    ...
    def create_from_memtable(
        cls, memtable: MemTable, filename: str, level: int = 0
    ) -> "SSTable":
...
class LSMTree:
    ...
    def __init__(
        self, base_dir: str = None, memtable_size: int = 100, max_levels: int = 4
    ):
    ...
    def _merge_sources(
        self, sources: List[Iterator[Tuple[str, str]]]
    ...
    def _maybe_compact(self) -> None:
        ...
        for level in range(len(self.levels) - 1):
            if len(self.levels[level]) > level_limits[level]:
                if (
                    self.compaction_thread is None
                    or not self.compaction_thread.is_alive()
                ):
